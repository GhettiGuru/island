...
class CrossProfileIntentFiltersHelper {
    ...
    public static void setFilters(PackageManager pm, int parentUserId, int managedProfileUserId) {
        ...
        capture.addAction(Intent.ACTION_MAIN);
        ...
        try {
            ...
            capture.addDataType("video/*");
            capture.addDataType("audio/*");
        } catch (IntentFilter.MalformedMimeTypeException e) {
            Log.e(TAG, "Mime type error: " + e.getMessage());
        }
        
        // Data schemes
        capture.addDataScheme("file");
        capture.addDataScheme("content");
        
        pm.addCrossProfileIntentFilter(capture, managedProfileUserId, parentUserId, 0);
        // ========== END CAMERA INTENT FILTER ==========
