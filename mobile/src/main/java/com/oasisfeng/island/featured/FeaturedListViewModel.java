...
public class FeaturedListViewModel extends AndroidViewModel {
	...
	public void update(final FragmentActivity activity) {
		...
		final LiveUserRestriction adb_secure = ! is_mainland_owner && ! has_profile ? null
				: LiveUserRestriction.create(app, DISALLOW_DEBUGGING_FEATURES, is_mainland_owner ? Users.getParentProfile() : Users.profile);
		if (adb_secure != null && (SHOW_ALL || dev_enabled || adb_secure.query(activity) == Boolean.TRUE)) {	// ADB is disabled so long as ADB secure is enabled.
